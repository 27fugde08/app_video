...
def main() -> None:
    ...
    parser.add_argument("--ffmpeg-path", type=str, default=None, help="Đường dẫn tùy chỉnh ffmpeg")
    parser.add_argument("--ffprobe-path", type=str, default=None, help="Đường dẫn tùy chỉnh ffprobe")
    ...
    merge_parser.add_argument("--vocal", required=True, type=str, help="Tệp audio giọng đọc")
    merge_parser.add_argument("--bgm", type=str, default=None, help="Tệp nhạc nền (BGM)")
    merge_parser.add_argument("--output", required=True, type=str, help="Tệp xuất ra (.mp3, .wav)")
    merge_parser.add_argument("--vocal-volume", type=float, default=1.0, help="Hệ số âm lượng giọng đọc")
    merge_parser.add_argument("--bgm-volume", type=float, default=0.3, help="Hệ số âm lượng nhạc nền")
    merge_parser.add_argument("--format", type=str, default="mp3", help="Định dạng xuất ra")
    ...
    mux_parser.add_argument("--audio", required=True, type=str, help="Đường dẫn audio lồng tiếng")
    mux_parser.add_argument("--subtitle", type=str, default=None, help="Đường dẫn tệp phụ đề (.srt)")
    mux_parser.add_argument("--output", required=True, type=str, help="Đường dẫn tệp video kết quả")
    mux_parser.add_argument("--subtitle-mode", choices=["soft", "hard"], default="soft", help="Chế độ phụ đề")

    convert_parser = subparsers.add_parser("convert-audio", help="Chuyển đổi định dạng và chuẩn hóa âm thanh")
    convert_parser.add_argument("--input", required=True, type=str, help="Tệp audio đầu vào")
    convert_parser.add_argument("--output", required=True, type=str, help="Tệp audio kết quả")
    convert_parser.add_argument("--sample-rate", type=int, default=44100, help="Tần số lấy mẫu")
    convert_parser.add_argument("--channels", type=int, default=2, help="Số kênh âm thanh")
    convert_parser.add_argument("--bitrate", type=str, default="320k", help="Bitrate xuất ra")
    convert_parser.add_argument("--lufs-norm", action="store_true", default=True, help="Kích hoạt LUFS normalization")
    ...
    if not engine.is_ffmpeg_available():
        ...
        print_status("LỖI HỆ THỐNG: Không tìm thấy công cụ FFmpeg.")
        sys.exit(1)
    ...
    try:
        ...
        logger.exception(f"Lỗi khi thực thi sub-command '{args.subcommand}': {e}")
