...
/// <summary>
/// Trạng thái của từng phân đoạn tải (Chunk state).
/// </summary>
public sealed class SegmentChunkInfo
{
    ...
    public long CurrentFileOffset => StartOffset + DownloadedBytes;
}

/// <summary>
/// File lưu trạng thái tải xuống phục vụ Pause / Resume (.download_state).
/// </summary>
...
/// <summary>
/// Báo cáo tiến độ tải thời gian thực (Zero-allocation struct).
/// </summary>
...
/// <summary>
/// Kết quả tải file hoàn tất.
/// </summary>
public sealed record SegmentDownloadResult(
    bool Success,
    string FilePath,
    long TotalBytes,
...
/// <summary>
/// Tùy chọn cấu hình cho FastSegmentDownloader.
/// </summary>
public sealed record SegmentDownloaderOptions
{
    /// <summary>
    /// Kích thước buffer đọc ghi từ ArrayPool (64KB theo yêu cầu).
    /// </summary>
    public int BufferSizeBytes { get; init; } = 64 * 1024;

    /// <summary>
    /// Số luồng tải tối đa cho file lớn (mặc định 8 luồng).
    /// </summary>
    public int MaxChunks { get; init; } = 8;

    /// <summary>
    /// Số luồng tối thiểu cho file trung bình (mặc định 4 luồng).
    /// </summary>
    public int MinChunks { get; init; } = 4;

    /// <summary>
    /// Giới hạn dung lượng tối thiểu để kích hoạt chia chunk (dưới 5MB sẽ tải đơn luồng).
    /// </summary>
    public long MinChunkSplitThresholdBytes { get; init; } = 5 * 1024 * 1024;

    /// <summary>
    /// Khoảng thời gian định kỳ cập nhật file trạng thái .download_state (ms).
    /// </summary>
    public int StateSaveIntervalMs { get; init; } = 500;

    /// <summary>
    /// Timeout cho mỗi request HTTP (giây).
    /// </summary>
