...
/// <summary>
/// Tiến trình bóc tách âm thanh theo thời gian thực (Zero-Allocation struct).
/// </summary>
...
/// <summary>
/// Kết quả sau khi tách thành công 2 stems độc lập.
/// </summary>
...
/// <summary>
/// Tham số cấu hình cho module Audio Stem Separator.
/// </summary>
public sealed record AudioStemSeparatorOptions
{
    /// <summary>
    /// Đường dẫn đến file mô hình MDX-Net ONNX (hoặc Demucs ONNX).
    /// </summary>
    public string ModelPath { get; init; } = "models/mdx_net_vocals.onnx";

    /// <summary>
    /// Tần số lấy mẫu chuẩn cho model MDX-Net (44100 Hz).
    /// </summary>
    public int SampleRate { get; init; } = 44100;

    /// <summary>
    /// Số kênh âm thanh (Stereo = 2 channels).
    /// </summary>
    public int Channels { get; init; } = 2;

    /// <summary>
    /// Kích thước cửa sổ biến đổi Fourier STFT (N_FFT = 2048 mẫu theo yêu cầu kỹ thuật).
    /// </summary>
    public int NFft { get; init; } = 2048;

    /// <summary>
    /// Bước nhảy khung STFT (Hop length = 512 mẫu, tương đương 75% overlap).
    /// </summary>
    public int HopLength { get; init; } = 512;

    /// <summary>
    /// Thời lượng mỗi chunk âm thanh xử lý (30 giây) để khống chế RAM dưới 250MB.
    /// </summary>
    public double ChunkDurationSeconds { get; init; } = 30.0;

    /// <summary>
    /// Thời gian chồng lấn (Crossfade Overlap = 1.0 giây) giữa các chunk nhằm khử tiếng click/pop tại mép nối.
    /// </summary>
    public double OverlapDurationSeconds { get; init; } = 1.0;

    /// <summary>
    /// Kích hoạt tăng tốc phần cứng DirectML (hoặc CUDA) qua GPU.
    /// </summary>
    public bool EnableDirectMl { get; init; } = true;

    /// <summary>
    /// Mã chỉ mục GPU phần cứng (mặc định GPU 0).
    /// </summary>
    public int GpuDeviceId { get; init; } = 0;

    /// <summary>
    /// Đường dẫn file thực thi ffmpeg để giải mã audio từ video nguồn.
    /// </summary>
