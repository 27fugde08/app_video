...
/// <summary>
/// Nền tảng video hỗ trợ quét hàng loạt.
/// </summary>
...
/// <summary>
/// Dữ liệu trích xuất hoàn chỉnh của từng video (Zero-Allocation metadata).
/// </summary>
...
/// <summary>
/// Tiến trình quét danh sách video theo thời gian thực.
/// </summary>
...
/// <summary>
/// Kết quả sau khi quét toàn bộ kênh/playlist.
/// </summary>
...
/// <summary>
/// Tùy chọn điều phối quét và phòng thủ chống chặn (Rate-Limit Defense).
/// </summary>
public sealed record ChannelScannerOptions
{
    /// <summary>
    /// Số lượng video tối đa cần quét (mặc định 50 video theo yêu cầu kiểm chứng).
    /// </summary>
    public int MaxVideosToFetch { get; init; } = 50;

    /// <summary>
    /// Kích thước mỗi trang pagination (mặc định 20-30 video/trang).
    /// </summary>
    public int PageSize { get; init; } = 20;

    /// <summary>
    /// Độ trễ ngẫu nhiên tối thiểu giữa các trang (Jitter Min = 800ms).
    /// </summary>
    public int MinJitterDelayMs { get; init; } = 800;

    /// <summary>
    /// Độ trễ ngẫu nhiên tối đa giữa các trang (Jitter Max = 2500ms).
    /// </summary>
    public int MaxJitterDelayMs { get; init; } = 2500;

    /// <summary>
    /// Thời gian tạm ngưng khi gặp lỗi HTTP 429 hoặc 403 (Backoff = 10 giây).
    /// </summary>
    public int RateLimitCooldownSeconds { get; init; } = 10;

    /// <summary>
    /// Số lần thử lại tối đa trước khi dừng.
    /// </summary>
